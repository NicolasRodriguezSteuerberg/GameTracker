...
public interface LibraryEntryRepository extends JpaRepository<LibraryEntryEntity, GameUserId>, JpaSpecificationExecutor<LibraryEntryEntity> {
    @Query("""
    SELECT new com.nsteuerberg.gametracker.shared.dto.CountGenresDTO(ge.id, ge.name, COUNT(le))
     ...
     """)
    List<CountGenresDTO> findGenresWithCount(Long userId);

    @Query("""
    SELECT new com.nsteuerberg.gametracker.shared.dto.CountPlatformsDTO(pl.id, pl.name, COUNT(le))
        FROM LibraryEntryEntity le
        JOIN le.game ga
     ...
     """)
    List<CountPlatformsDTO> findPlatformsWithCount(Long userId);
